# ==========================================
# AMAZON E-COMMERCE DASHBOARD (FIXED)
# ==========================================
...
tab1, tab2, tab3, tab4 = st.tabs(["Sales", "Brands", "Customers", "Insights"])

with tab1:

    category_sales = filtered_df.groupby('category')['final_price'].sum().reset_index()

    fig = px.bar(category_sales, x='category', y='final_price', template=PLOT_THEME)
...
with tab2:
    ...
    fig = px.bar(brand_sales.head(10), x='brand', y='final_price', template=PLOT_THEME)
    st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    fig = px.pie(payment_data, names='payment_method', values='final_price', hole=0.5)
    st.plotly_chart(fig, use_container_width=True)
